...
def train_model(model, dataloaders, criterion, optimizer, num_epochs=25):
    cca = CCA(n_components=2)  # Instantiate CCA once
    ...
    for epoch in range(num_epochs):
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                combined_inputs = np.hstack((social, wearable, academic, peer))
                
                # Apply CCA
                cca.fit(combined_inputs, labels)
                X_cca, _ = cca.transform(combined_inputs, labels)
